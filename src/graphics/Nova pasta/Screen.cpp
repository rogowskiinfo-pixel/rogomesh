...
// ==========================================
// ROGOMESH: Inicializacao do estado da tela
// O dispositivo sempre ligara na tela de Sistema
// ==========================================
RogoScreenState currentRogoState = ROGO_SCREEN_SYSTEM;
// ==========================================
...
{
    // ROGOMESH: Desativado para carregar a interface RogoMesh direto no boot
    /*
    if (address_found.address) {
...
// ==========================================
    // ROGOMESH: Sequestro do Carrossel
    // Ignora todas as telas velhas e define a nossa como unica!
    // ==========================================
    numframes = 0;
    normalFrames[numframes++] = &graphics::Screen::drawRogoMeshScreen;
    // ==========================================

    ui->setFrames(normalFrames, numframes);
    ui->enableAllIndicators();
    // ROGOMESH: Força o display a desenhar o nosso quadro imediatamente
    ui->update();
...
void Screen::handleOnPress()
{
    ...
    // ==========================================
    // ROGOMESH: Alternador de Telas
    // ==========================================
    // Avança 1 estado. O "% 6" garante que se passar da tela 5, volte para a 0!
    currentRogoState = static_cast<RogoScreenState>((currentRogoState + 1) % 6);
    
    // Força o display a atualizar o desenho imediatamente ao clicar
    if (ui) {
        ui->update(); 
    }
    // ==========================================

    // ... (o resto do código original continua aqui para baixo) ...
...
// ==========================================
// ROGOMESH: O Diretor Principal da Interface
// ==========================================
void graphics::Screen::drawRogoMeshScreen(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    // Limpa qualquer configuração anterior de texto e seta o padrão
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    display->setFont(FONT_SMALL);

    // O "Semáforo": decide o que desenhar baseado no estado atual
    switch (currentRogoState) {
        case ROGO_SCREEN_SYSTEM:
            display->setFont(FONT_MEDIUM);
            display->drawString(x + 5, y + 5, "ROGOMESH v1.0");
            display->setFont(FONT_SMALL);
            display->drawString(x + 5, y + 30, "Status: ONLINE");
            display->drawString(x + 5, y + 45, "Placa: Heltec V2");
            break;

        case RogoScreenState::ROGO_SCREEN_NETWORK: {
            uint32_t totalNodes = 0;
            if (nodeDB) {
                totalNodes = nodeDB->getNumMeshNodes();
            }
            
            display->drawString(x + 64, y + 16, "Nodes: " + String(totalNodes));
            display->drawString(x + 64, y + 32, "Status: Online");
            break;
        }
        case RogoScreenState::ROGO_SCREEN_GPS: {
            String sats = "0";
            String fixStatus = "Buscando...";
            
            if (gpsStatus) {
                sats = String(gpsStatus->getNumSatellites());
                if (gpsStatus->getHasLock()) {
                    fixStatus = "Fix OK!";
                }
            }
            
            display->drawString(x + 64, y + 16, "Sats: " + sats);
            display->drawString(x + 64, y + 32, fixStatus);
            break;
        }
        case RogoScreenState::ROGO_SCREEN_ENERGY: {
            String batPercent = "0%";
            String batVolt = "0.00 V";

            if (powerStatus) {
                batPercent = String(powerStatus->getBatteryChargePercent()) + "%";
                batVolt = String(powerStatus->getBatteryVoltageMv() / 1000.0, 2) + " V";
            }
            
            display->drawString(x + 64, y + 16, "Carga: " + batPercent);
            display->drawString(x + 64, y + 32, "Volt: " + batVolt);
            break;
        }

       case RogoScreenState::ROGO_SCREEN_RADIO:
            display->drawString(x + 5, y + 20, ">> RADIO LORA <<");
            display->drawString(x + 5, y + 35, "Freq: 915 MHz");
            break;

        case RogoScreenState::ROGO_SCREEN_DIAGNOSTIC:
            display->drawString(x + 5, y + 20, ">> DIAGNOSTICO <<");
            display->drawString(x + 5, y + 35, "Tudo Normal");
            break;
    }
}
// ==========================================